...
if os.environ.get('FLASK_DEBUG') == 'development':
    try:
        from admin_dev_access import admin_dev_bp
        app.register_blueprint(admin_dev_bp)
        app.logger.info("Development admin access blueprint registered successfully")
    except ImportError as e:
        app.logger.warning(f"Could not register development admin blueprint: {str(e)}")
