# September 2026 — Unified Marketing Calendar (posts + events + outreach + ops)
...
- **Recap-numbered posts (#X-12 etc.) get REAL current numbers** at post time, not the library's placeholder week counts.
- **Claim discipline:** API/options/prediction markets = "on the roadmap" only; never cite subscriber/revenue aggregates without netting gifted counts; UC-D never promised publicly.
- **Bots:** trigger-driven only, no calendar batches (freeze coming 10/15).
- **Press (GME/AMC writers CSV, 33 targets):** September is **warm-up only** — follow + genuinely engage from the founder X account. NO pitches until press-kit beats (Oct) / launch window. Pitching cold in Sep burns the one shot.
...
- **Press pitches** (Matt Levine, Sherwood pair, Banerji, Kochkodin + 28 more) — Oct press-kit beats at earliest; warm-up only in Sep
- **R-02 r/algotrading** — fires when UC-A API keys exist, not before
- **LinkedIn** — quiet per master timeline
- **Bot creation** — trigger-only; hard freeze 10/15
- **Premium+ decision** — mid-Oct

## October preview (so nothing sneaks up)
Stocktoberfest Oct 5–7 (shirts on, cards loaded, bike outside) · QWAFAxNEW Petter Kolm event (RSVP on announce) · press-kit beats begin · R-04 #2 ~10/8 · bot freeze 10/15 · early access ends 10/14 · Premium+ decision mid-Oct.
