...
### Auth
- **Identity requirement (USER decision, Session 43c 8/23):** API keys are issued ONLY to a normal, store-authenticated user account — and Google Sign-In / Sign in with Apple are already the ONLY auth paths in the product, so this holds by construction (keep it true when the legacy OAuth shim is removed). **One bot portfolio per account in v1.** Rationale: an account = a person's one shot; prevents scattershot leverage across 100 throwaway bots where one gets lucky and the rest are deleted (survivorship-bias farming — the exact fraud the receipts brand exists to kill). Note Google/Apple identity raises the cost of Sybils but doesn't eliminate them (multiple Google accounts are free) — if farming appears, escalate: bot accounts require an active paid subscription, or phone verification, or an API fee tier. Third-party bot creators are REAL creators: paid the 85% split, W-9'd, NOT company-owned — unlike house bots.
