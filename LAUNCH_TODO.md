...
- [x] **Cash-Tracking-Drift + Snapshot-Drift cards both persist + reload correctly.** ✅ Shipped this session. Root cause was the inline `User.query.filter_by(email=ADMIN_EMAIL).first()` lookup: when ADMIN_EMAIL on Vercel didn't match the DB row's email exactly (casing/whitespace), the lookup returned None and the entire persistence block was silently skipped — auth had already passed via the `current_user.email` fallback so the request still 200'd, but `admin.extra_data['last_drift_check']` was never written, so next page-load returned `never_run`. Fix: new `_find_admin_user_for_persistence()` helper in `admin_cash_tracking.py` tries `current_user` (Flask-Login) → case-insensitive email match → exact match, with a WARNING log on miss. Both writers (drift-check, snapshot-audit) and both readers (last-drift-check, last-snapshot-audit) now use this helper, so writer/reader can never disagree. Also made `cron_snapshot_audit` dual-auth so the new "Run now" button on the Snapshot Drift card works from a browser session (previously cron-secret-only).
