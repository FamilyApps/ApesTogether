...
- [ ] **Fix stale Firebase bundle ID in iOS `GoogleService-Info.plist`** (discovered May 11 while configuring strict OAuth verification). The plist's `BUNDLE_ID` field reads `com.apestogether.app`, but the actual app's `PRODUCT_BUNDLE_IDENTIFIER` in `ios/ApesTogether/ApesTogether.xcodeproj/project.pbxproj` is `com.apestogether.ApesTogether`. This mismatch likely means FCM iOS push delivery is silently broken — APNs token registration would fail when Firebase tries to map the device token to a `com.apestogether.app` bundle that no Firebase app config actually represents. Fix: download a fresh `GoogleService-Info.plist` from the Firebase Console iOS app entry (or create a new iOS app there with bundle ID `com.apestogether.ApesTogether` and replace the plist). May explain the historical "bobford00 never gets push notifications" observation. Same concern for `APPLE_BUNDLE_ID` default in `iap_validation_service.py` — already uses the correct value; just noting consistency.
- [~] **🚨 SECURITY (pre-launch blocker): Verify Google/Apple ID tokens properly.** Code shipped May 11 — `mobile_api.py` now contains `_verify_google_id_token` / `_verify_apple_id_token` helpers using PyJWT's `PyJWKClient` to fetch + cache Google's and Apple's JWKS and verify signature + issuer + audience + expiration. Gated behind `STRICT_OAUTH_VERIFICATION` env var (default off → legacy decode-without-verification path kept for rollout safety; CRITICAL log line emitted on every legacy auth request so the gap is visible in Vercel logs). **Still TODO**: set `GOOGLE_IOS_CLIENT_ID` + `GOOGLE_ANDROID_CLIENT_ID` + `APPLE_BUNDLE_ID` on Vercel (see `ENV_VARIABLES.md` § OAuth ID Token Verification for the rollout playbook), then `STRICT_OAUTH_VERIFICATION=enforce`, redeploy, smoke-test Google sign-in on iOS + Android and Apple sign-in on iOS, then delete the legacy path in a follow-up commit.
