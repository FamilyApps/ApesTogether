...
- [ ] **POST-LAUNCH CLEANUP**: delete dead admin-stub files that aren't reachable on production but clutter the codebase: `admin_route_debug.py` (+ its registration block in `app.py:748-754`), `api/admin.py`, `api/debug.py`. Verified harmless during May 12 security audit — Vercel routes everything through `api/vercel.py` → `api/index.py`, which doesn't import any of them. Deletion is pure hygiene.
