...
- [ ] **#4 open-time anomaly (apes1575 spike / divi51 dip).** Reported Jun 1 — a visible jump right around the 9:30 ET open that snaps back. Distinct from the EOD-wave false positives cleared by `7549f80` (those were post-20:00 UTC). Capture the intraday snapshot + price values around open to find the bad data point.
...
- [ ] **ONE-TIME DATA FIX: bobford00's stuck after-hours META×5.** Created BEFORE the #3 fix via the Buy sheet, so it's a live `Stock` row at `purchase_price=0` with NO `stock_transaction`. It will NOT self-resolve at open (nothing is queued for it; the #3 code only changes future trades). Decide: (a) convert to a pending queued buy that settles next open, (b) delete it, or (c) backfill a real cost basis + buy txn. Direct-SQL fix, no deploy (mirrors the zen1889 pattern).
...
- **New findings logged (Sections A/B/H):** (1) bobford00's pre-fix after-hours META×5 is stuck in Holdings at $0 basis and needs a one-time data fix; (2) `add_stocks` seed path records `price=0`/no txn (cost-basis bug); (3) #4 apes1575/divi51 open-time anomaly; (4) #1 bot-rule verification + #2 per-sector data adequacy still open; (5) on-device verification of pending render pending.
