...
class W9Submission(db.Model):
    """IRS Form W-9 submissions from Content Creators for 1099-NEC reporting.
    
    SSN/EIN is encrypted at rest using Fernet symmetric encryption.
    The encryption key is stored in the ENCRYPTION_KEY env var (never in code).
    Only the last 4 digits of the TIN are stored in plain text for admin display.
    """
    __tablename__ = 'w9_submission'
    ...
    business_name = db.Column(db.String(200), nullable=True)  # DBA or disregarded entity name
    
    # Federal tax classification
    tax_classification = db.Column(db.String(50), nullable=False)
    # 'individual', 'sole_proprietor', 'llc_single', 'llc_partnership',
    # 'llc_c_corp', 'llc_s_corp', 'c_corp', 's_corp', 'partnership', 'trust_estate', 'other'
    
    # Address
    address_line1 = db.Column(db.String(200), nullable=False)
    address_line2 = db.Column(db.String(200), nullable=True)
    city = db.Column(db.String(100), nullable=False)
    state = db.Column(db.String(2), nullable=False)  # 2-letter state code
    zip_code = db.Column(db.String(10), nullable=False)  # ZIP or ZIP+4
    
    # Taxpayer Identification Number (encrypted)
    tin_type = db.Column(db.String(3), nullable=False)  # 'ssn' or 'ein'
    tin_encrypted = db.Column(db.LargeBinary, nullable=False)  # Fernet-encrypted SSN/EIN
    tin_last_four = db.Column(db.String(4), nullable=False)  # Last 4 digits for admin display
    
    # Electronic signature
    signature_name = db.Column(db.String(200), nullable=False)  # Typed full legal name
    signature_date = db.Column(db.Date, nullable=False)
    signature_ip = db.Column(db.String(45), nullable=True)  # IP address at time of signing
    ...
    reviewed_by = db.Column(db.String(100), nullable=True)  # Admin email
    ...
    # Relationship
    user = db.relationship('User', backref=db.backref('w9_submissions', lazy='dynamic'))
    
    @property
    def is_active(self):
        """A W-9 is active if it's submitted or verified and not superseded"""
        return self.status in ('submitted', 'verified')
    
    @property
    def display_tin(self):
        """Display-safe TIN: ***-**-1234 or **-***1234"""
        if self.tin_type == 'ssn':
            return f"***-**-{self.tin_last_four}"
        return f"**-***{self.tin_last_four}"
    
    def __repr__(self):
        return f"<W9Submission user={self.user_id} status={self.status} tin=...{self.tin_last_four}>"
