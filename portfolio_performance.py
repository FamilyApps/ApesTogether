        ...
        try:
            ...
            for date_str, price_data in time_series.items():
                try:
                    ...
                    # INSURANCE: if AV's daily series ever includes TODAY as a
                    # row valued at the as-of-fetch-time price while the market
                    # is open, the insert-only logic below would freeze that
                    # intraday price as the official close forever. An 8/25/26
                    # investigation showed intraday fetches had NOT actually
                    # persisted such rows that day, but the failure mode is
                    # real and silent, so the guard stays: skip today's row
                    # until it is final -- 21:05 UTC covers the 4 PM ET close
                    # in both EDT and EST; the 23:00 UTC nightly map/AV cron
                    # backfills it the same day. Verification/repair tool:
                    # /admin/refresh-daily-closes.
