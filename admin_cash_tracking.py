    ...
    def drift_check_cash_tracking():
        ...
        try:
            ...
            for user in users:
                ...
                if seeded_baseline < 0.01 and drift_sell_count == 0:
                    # Cost-basis fallback ONLY for sell-free ledgers: realized
                    # proceeds can be redeployed, so cost_basis > replay_max is
                    # NOT evidence of seeding once sells exist (same guard as
                    # _full_rebuild_for_user; the unguarded version here produced
                    # the 8/23 chart1658 false-positive drift email).
                    stocks = Stock.query.filter_by(user_id=user.id).all()
